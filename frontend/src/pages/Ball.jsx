...
export default function Ball() {
  const { data, error, status, analysis, mode, setMode, model } = useBallAnalytics()
  ...
  return (
    <ReplayPage status={status} analysis={analysis} error={error}>
      <div className="anim-fade-in">
        <BallHeader data={data} model={model} onUpload={analysis.handleAnalysisStart} />

        <main className="mx-auto max-w-7xl space-y-6 px-8 py-8">
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1.55fr_0.85fr]">
            <BallHeatmapSection data={data} mode={mode} model={model} onModeChange={setMode} />
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-[0.95fr_1.05fr]">
            ...
            <TerritoryPanel territoryData={model.territoryData} thirdsData={model.thirdsData} />
          </section>

          <BallBandPanels speedData={model.speedData} heightData={model.heightData} />

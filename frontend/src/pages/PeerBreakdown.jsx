  ...
  return (
    <div className="anim-fade-in">
      ...
      >
        ...
        {summary && (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <HeroMetric
              label="Matches"
              value={fmt(summary.totalMatches)}
              detail="selected player sample"
              color={BLUE}
              Icon={Activity}
            />
            <HeroMetric
              label="Teammates"
              value={fmt(summary.uniqueTeammates)}
              detail={`${fmt(teammateGames)} teammate pairings`}
              color={GREEN}
              Icon={ShieldCheck}
            />
            <HeroMetric
              label="Opponents"
              value={fmt(summary.uniqueOpponents)}
              detail={`${fmt(opponentGames)} opponent pairings`}
              color={RED}
              Icon={Swords}
            />
            <HeroMetric
              label="Best Teammate"
              value={
                summary.bestTeammate
                  ? shortName(summary.bestTeammate.playerName, 12)
                  : "N/A"
              }
              detail={
                summary.bestTeammate
                  ? `${fmtPct(summary.bestTeammate.winRate)} across ${fmt(summary.bestTeammate.matches)}`
                  : "No teammate samples"
              }
              color={GOLD}
              Icon={Trophy}
            ...
            <HeroMetric
              ...
              value={
                summary.toughestOpponent
                  ? shortName(summary.toughestOpponent.playerName, 12)
                  : "N/A"
              }
              detail={
                summary.toughestOpponent
                  ? `${fmtPct(summary.toughestOpponent.winRate)} win rate against`
                  : "No opponent samples"
              }
              color={PURPLE}
              Icon={Crosshair}
      ...
      {isLoading && (
        <CareerEmptyState
          Icon={Users}
          title="Loading relationship breakdown"
          detail="Aggregating same-team and opposite-team samples from indexed replays..."
          tone={BLUE}
        />
      )}
      {!isLoading && error && (
      ...
      {!isLoading && !error && !summary && (
        <CareerEmptyState
          Icon={Users}
          title="No peer data yet"
          detail="Import multi-player replays to populate teammate and opponent splits."
          tone={GOLD}
        />
      )}
        ...
        <main className="mx-auto max-w-7xl space-y-6 px-8 py-8">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
            <PeerSpotlight
              ...
              peer={summary.mostPlayedWith}
              icon={ShieldCheck}
              color={GREEN}
              fallback="No teammate"
            />
            <PeerSpotlight
              title="Most played against"
              peer={summary.mostPlayedAgainst}
              icon={Swords}
              color={RED}

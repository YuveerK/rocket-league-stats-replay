...
export default function PlayerCompare() {
  const {
    players, playerA, setPlayerA, playerB, setPlayerB,
    activeCompare, commonMaps, commonTeammates, commonOpponents, sharedMatches,
    isLoading, needsSecondPlayer, samePlayer, error,
  } = usePlayerCompare()
    ...
    <div className="anim-fade-in">
      ...
      >
        <ComparePicker
          ...
          onPlayerA={setPlayerA} onPlayerB={setPlayerB}
      ...
      {activeCompare && !isLoading && !error && !samePlayer && (
        <main className="mx-auto max-w-7xl space-y-6 px-8 py-8">
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <ComparePlayerCard player={activeCompare.playerA} color={BLUE}   side="Player A" />
            <ComparePlayerCard player={activeCompare.playerB} color={PURPLE} side="Player B" />
          </div>

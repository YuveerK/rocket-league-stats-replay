...
function teamColor(team) {
  return TEAM_COLORS[team] ?? "rgba(255,255,255,0.45)";
}
function teamLabel(team) {
  return TEAM_LABELS[team] ?? "Unknown";
}

function bandBg(team, i) {
  return (team === 0 ? BLUE : ORANGE) + BAND_BG_OPACITIES[i];
}
function bandText(team, i) {
  return (team === 0 ? BLUE : ORANGE) + BAND_TEXT_OPACITIES[i];
}
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {players.map((player) => (
        <span
          key={player.key}
          className="inline-flex items-center gap-2 rounded-full border border-white/8 bg-white/[0.035] px-2.5 py-1 text-[11px] font-bold text-white/55"
        >
          <span
    ...
    <div className="grid grid-cols-1 gap-3 lg:grid-cols-2 2xl:grid-cols-3">
      {players.map((player) => {
        ...
        return (
          <article
            key={player.key}
            className="relative overflow-hidden rounded-2xl border bg-white/2.5 p-4"
            style={{ borderColor: `${tc}22` }}
          >
            <div
            ...
            <div className="relative flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <h4 className="truncate text-sm font-black text-white/82">
                    {player.playerName}
                  </h4>
                </div>
                <p className="mt-1 text-xs text-white/32">
                  {teamLabel(player.team)} team · {player.car ?? "Unknown car"}
                </p>
              </div>
              <span
                className="shrink-0 rounded-lg border px-2 py-1 text-[10px] font-black uppercase tracking-widest"
                style={{
                  borderColor: `${tc}25`,
                  color: tc,
                  background: `${tc}12`,
            ...
            <div className="relative mt-4 grid grid-cols-3 gap-3 sm:grid-cols-5">
              <div>
                ...
                <div
                  className="stat-num mt-1 text-lg font-black"
                  style={{ color }}
                >
                  {fmt(player.bpm, 1)}
                </div>
              ...
              <div>
                ...
                <div className="stat-num mt-1 text-lg font-black text-emerald-300">
                  {fmt(player.boostCollectedApprox)}
                </div>
              </div>
              <div>
                <div className="section-label">Big</div>
                <div className="stat-num mt-1 text-lg font-black text-sky-300">
                  {fmt(player.bigPads)}
                </div>
              </div>
              <div>
                <div className="section-label">Small</div>
                <div className="stat-num mt-1 text-lg font-black text-violet-300">
                  {fmt(player.smallPads)}
                </div>
              </div>
              <div>
                <div className="section-label">Stolen</div>
                <div className="stat-num mt-1 text-lg font-black text-amber-300">
            ...
            <div className="relative mt-4 h-1.5 overflow-hidden rounded-full bg-white/7">
              <div
                ...
                style={{
                  width: `${Math.min(100, (n(player.bpm) / maxBpm) * 100)}%`,
                  background: color,
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
        {BAND_LABELS.map((label, i) => (
          <div key={label} className="flex items-center gap-1.5">
            <div
              ...
              style={{
                background: `linear-gradient(90deg, ${bandBg(0, i)}, ${bandBg(1, i)})`,
              }}
            />
        ...
        <span className="ml-auto text-[10px] text-white/20 italic">
          brighter = more time at high boost
        </span>
      </div>

      {players.map((player) => {
        const totalSec =
          n(player.boost0To25Seconds) +
          n(player.boost25To50Seconds) +
          n(player.boost50To75Seconds) +
          n(player.boost75To100Seconds);
        return (
          <div key={player.key}>
            <div className="mb-2.5 flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span
                  ...
                  style={{
                    color: teamColor(player.team),
                    background: `${teamColor(player.team)}18`,
                  }}
                >
                  {teamLabel(player.team)}
                </span>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: bandBg(player.team, i),
                    }}
              ...
              {BAND_KEYS.map((key, i) => (
                <div key={key} className="min-w-0 text-center">
                  <div
                    ...
                    style={{ color: bandText(player.team, i) }}
                  >
                    {fmtPct(n(player[key]))}
                  </div>
                  <div className="text-[9px] tracking-wide text-white/25">
                    {BAND_LABELS[i]}
                  </div>
                </div>
              ))}
      ...
      <table className="min-w-387.5 w-full border-collapse text-left">
        <thead>
          <tr className="border-b border-white/7 bg-white/[0.035]">
            <th className="sticky left-0 z-10 bg-[#0d111d] px-4 py-3 text-xs font-black uppercase tracking-widest text-white/42">
              Player
            </th>
            {columns.map(([label]) => (
              <th
                key={label}
                className="px-3 py-3 text-right text-xs font-black uppercase tracking-widest text-white/34"
              >
                {label}
              </th>
            ))}
        ...
        <tbody>
          {players.map((player) => (
            <tr
              key={player.key}
              className="border-b border-white/5.5 last:border-b-0"
            >
              <td className="sticky left-0 z-10 bg-[#0b0f1a] px-4 py-3">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  />
                  <div className="min-w-0">
                    <div className="truncate text-sm font-black text-white/78">
                      {player.playerName}
                    </div>
                    <div
              ...
              {columns.map(([label, render]) => (
                <td
                  key={label}
                  className="stat-num px-3 py-3 text-right text-xs font-bold text-white/64"
                >
                  {render(player)}
                </td>
              ))}
...
export default function BoostPlayers() {
  const { data, loading, error } = usePageData("/api/boost-players");

  const status = loading ? "loading" : error || !data ? "empty" : "ready";

  const players = useMemo(() => {
    const sorted = [...(data?.players ?? [])].sort(
      (a, b) => n(a.team) - n(b.team) || n(b.bpm) - n(a.bpm),
    );
    const teamCounts = {};
  ...
  const economyRows = useMemo(
    () =>
      makeRows(players, [
        { label: "BPM", value: (p) => p.bpm },
        { label: "Avg boost", value: (p) => p.averageBoost },
      ]),
    [players],
  );
  const reserveRows = useMemo(
    () =>
      makeRows(players, [
        { label: "0 boost", value: (p) => p.zeroBoostSeconds },
        { label: "100 boost", value: (p) => p.fullBoostSeconds },
      ]),
    [players],
  );
  const supersonicRows = useMemo(
    () =>
      makeRows(players, [
        {
          label: "Used while supersonic",
          value: (p) => p.boostUsedWhileSupersonic,
        },
      ]),
    [players],
  );
  const padRows = useMemo(
    () =>
      makeRows(players, [
        { label: "Big pads", value: (p) => p.bigPads },
        { label: "Stolen big", value: (p) => p.stolenBigPads },
        { label: "Small pads", value: (p) => p.smallPads },
        { label: "Stolen small", value: (p) => p.stolenSmallPads },
      ]),
    [players],
  );
  const amountRows = useMemo(
    () =>
      makeRows(players, [
        { label: "Big collected", value: (p) => p.amountCollectedBigPads },
        { label: "Big stolen", value: (p) => p.amountStolenBigPads },
        { label: "Small collected", value: (p) => p.amountCollectedSmallPads },
        { label: "Small stolen", value: (p) => p.amountStolenSmallPads },
      ]),
    [players],
  );
  const overfillRows = useMemo(
    () =>
      makeRows(players, [
        { label: "Overfill total", value: (p) => p.overfillTotal },
        { label: "From stolen", value: (p) => p.overfillFromStolen },
      ]),
    [players],
  );

  const highestBpm = players.reduce(
    (best, p) => (n(p.bpm) > n(best?.bpm) ? p : best),
    players[0],
  );
  const mostOverfill = players.reduce(
    (best, p) => (n(p.overfillTotal) > n(best?.overfillTotal) ? p : best),
    players[0],
  );
  ...
  const topPadCollector = players.reduce(
    (best, p) => (n(p.pickups) > n(best?.pickups) ? p : best),
    players[0],
  );
      ...
      <div className="anim-fade-in">
        ...
        >
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <HeroMetric
              ...
              detail="Tracked from replay PRI data"
              color={BLUE}
              Icon={Database}
            />
            <HeroMetric
              ...
              detail="Large pad pickups (match total)"
              color={BLUE}
            ...
            <HeroMetric
              ...
              detail="Small pad pickups (match total)"
              color={ORANGE}
              Icon={Layers}
            />
            <HeroMetric
              label="Top Collector"
              value={shortName(topPadCollector?.playerName, 12)}
              detail={`${fmt(topPadCollector?.pickups)} pads · ${fmt(topPadCollector?.bigPads)} big`}
              color={topPadCollector?.color ?? BLUE}
              Icon={Gauge}
            />
            <HeroMetric
              label="BPM Leader"
              value={shortName(highestBpm?.playerName, 12)}
              detail={`${fmt(highestBpm?.bpm, 1)} boost per minute`}
              color={highestBpm?.color ?? BLUE}
              Icon={Gauge}
            />
            <HeroMetric
              label="Total Stolen"
              value={fmt(totalStolen)}
              detail="Other-side pickup boost (units)"
              color={ORANGE}
              Icon={ShieldAlert}
            />
            <HeroMetric
              label="Overfill Leader"
              value={shortName(mostOverfill?.playerName, 12)}
              detail={`${fmt(mostOverfill?.overfillTotal)} boost lost to cap`}
              color={BLUE}
              Icon={BatteryCharging}
            />
          </div>
          ...
          <div className="flex flex-wrap gap-2 text-xs text-white/35">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-white/7 bg-white/4 px-2.5 py-1">
              <Clock size={12} /> {fmtSeconds(data?.matchDuration)} match time
            </span>
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-white/7 bg-white/4 px-2.5 py-1">
              <Upload size={12} /> {data?.mapName ?? "Unknown map"}
            </span>
          </div>
        </PageHeader>
        ...
        <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-8 sm:py-8">
          <Panel
            ...
            subtitle="BPM, average boost and stolen boost at a glance"
            Icon={Gauge}
            accent={BLUE}
          >
          ...
          <Panel
            eyebrow="Economy"
            title="Boost Per Minute And Average Amount"
            subtitle="Primary player boost economy metrics"
            Icon={Database}
            accent={BLUE}
          >
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-2">
            <Panel
              ...
              title="Seconds At 0 And 100 Boost"
              subtitle="Time spent starved or capped"
              Icon={BatteryCharging}
              accent={BLUE}
            >
              <PlayerLegend players={players} />
              <div className="mt-5">
                <GroupedPlayerBar
                  ...
                  rows={reserveRows}
                  height={310}
                  formatter={fmtSeconds}
                  yFormatter={(v) => `${fmt(v)}s`}
                />
              </div>
            </Panel>
            <Panel
              eyebrow="Supersonic Spend"
              title="Amount Used While Supersonic"
              subtitle="Boost decreases while car is at supersonic speed"
              Icon={Zap}
              accent={ORANGE}
            >
              <PlayerLegend players={players} />
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1.05fr_0.95fr]">
            <Panel
              eyebrow="Boost Ranges"
              title="Time Spent In Boost Bands"
              subtitle="Percent of player boost sample time in each band"
              Icon={PackagePlus}
              accent={BLUE}
            >
            ...
            <Panel
              ...
              subtitle="Stolen means other team's side; midfield pickups are excluded"
              Icon={ShieldAlert}
              accent={ORANGE}
            >
              <PlayerLegend players={players} />
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-2">
            <Panel
              eyebrow="Collected Amounts"
              title="Big And Small Pad Amounts"
              subtitle="Visible boost gained from pad pickups, split by stolen and non-stolen totals"
              Icon={Database}
              accent={BLUE}
            >
              <PlayerLegend players={players} />
            ...
            <Panel
              eyebrow="Overfill"
              title="Boost Lost To The Cap"
              subtitle="At 80 boost, collecting a big pad overfills by 80"
              Icon={BatteryCharging}
              accent={ORANGE}
            >
              <PlayerLegend players={players} />
          ...
          <Panel
            eyebrow="Reference"
            title="Player Boost Stat Sheet"
            subtitle="Every requested boost player metric in one table"
            Icon={Layers}
            accent={BLUE}
          >
            <DetailTable players={players} />
            <div className="mt-4 grid grid-cols-1 gap-3 text-xs text-white/34 md:grid-cols-3">
              <p className="rounded-xl border border-white/7 bg-white/2.5 p-3">
                <span className="font-black text-white/55">Stolen:</span>{" "}
                pickups on the other team's side only. Middle boosts are not
                counted.
              </p>
              <p className="rounded-xl border border-white/7 bg-white/2.5 p-3">
                <span className="font-black text-white/55">Overfill:</span> pad
                value minus visible boost gain, so 80 boost plus a big pad
                wastes 80.
              </p>
              <p className="rounded-xl border border-white/7 bg-white/2.5 p-3">
                <span className="font-black text-white/55">Ranges:</span>{" "}
                percentages are time-weighted from each player's replicated
                boost meter.
              </p>
            </div>
          </Panel>

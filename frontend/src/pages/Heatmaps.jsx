  ...
  return (
    ...
    >
      ...
      <div className="flex items-end gap-1">
        <span className="text-2xl font-black stat-num" style={{ color }}>
          {value}
        </span>
...
export default function Heatmaps() {
  ...
  useEffect(() => {
    apiGet("/api/heatmap-data")
      .then((d) => {
        ...
        setVisiblePlayers(
          d.players[0]?.playerName ? [d.players[0].playerName] : [],
        );
      })
  ...
  const player =
    data.players.find((p) => p.playerName === focusedName) ?? data.players[0];
  if (!player) return null;

  const team0 = data.players.filter((p) => p.team === 0);
  const team1 = data.players.filter((p) => p.team === 1);
  const pColor =
    colorByName.get(player.playerName) ?? TEAM_COLOR[player.team] ?? "#60a5fa";
    ...
    <div className="anim-fade-in">
      ...
      <div
        ...
        style={{
          ...
          background:
            "linear-gradient(135deg,rgba(37,99,235,0.08) 0%,#05070f 40%,#05070f 60%,rgba(234,88,12,0.08) 100%)",
        }}
      >
        ...
        <div className="flex gap-1.5 flex-wrap">
          {team0.map((p) => (
            <PlayerTab
              key={p.playerName}
              player={p}
              focused={focusedName === p.playerName}
          ...
          <div className="card p-0 overflow-hidden">
            ...
            >
              ...
              <span className="text-[10px] text-white/25 stat-num">
                {visiblePlayerList.length} layer
                {visiblePlayerList.length === 1 ? "" : "s"}
              </span>
            </div>
            ...
            >
              ...
              <p className="mt-2 text-center text-[10px] text-white/30">
                {showTrail
                  ? "Trail overlay on · toggle off for density only"
                  : "Brighter areas = more time spent"}
              </p>
              <div className="flex flex-wrap gap-1.5 mt-3">
                {visiblePlayerList.map((p) => {
                  const color =
                    colorByName.get(p.playerName) ??
                    TEAM_COLOR[p.team] ??
                    "#60a5fa";
                  return (
                    <button
                ...
                {ZONE_GROUPS.map((group) => (
                  <section key={group.key} className="space-y-3">
                    ...
                    <div className={`grid ${group.columns} gap-2.5`}>
                      {ZONE_CONFIG.filter(
                        ({ group: zoneGroup }) => zoneGroup === group.key,
                ...
                {data.players.map((p) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span
                        className={`text-sm font-medium min-w-0 truncate text-left ${isSel ? "text-white" : "text-white/55"}`}
                      >
                        {p.playerName}
                      </span>

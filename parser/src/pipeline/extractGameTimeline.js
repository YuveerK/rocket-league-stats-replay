  ...
  for (const sample of clockSamples) {
    ...
    if (previousSample) {
      const clockStartedCountingUp =
        lowestSeenClock <= 1 &&
        gameSecondsRemaining > previousSample.gameSecondsRemaining;
      const jumpedFromRegulationEnd =
        lowestSeenClock <= 5 &&
        gameSecondsRemaining - previousSample.gameSecondsRemaining > 5;

      if (clockStartedCountingUp || jumpedFromRegulationEnd) {
        return sample;
      }
    }
...
function applyOvertimeClockElapsed(events, overtimeStartSample, initialClockSeconds) {
  ...
  for (const event of events) {
    ...
    const gameClockElapsedSeconds =
      initialClockSeconds + event.gameSecondsRemaining;
...
function buildCountdownIntervals(stateSamples, finalElapsedSeconds) {
  ...
  // Keep the initial kickoff countdown visible at the start of the rail. Later
  // countdowns are inter-round dead time and make events drift right.
...
function timelineElapsedSeconds(elapsedSeconds, ignoredIntervals) {
  ...
  for (const interval of ignoredIntervals) {
    ...
    ignoredSeconds +=
      Math.min(elapsedSeconds, interval.end) - interval.start;
  }

  return roundSeconds(elapsedSeconds - ignoredSeconds);
}

function buildClockPlaybackSamples({
  clockSamples,
  firstFrameTime,
  initialClockSeconds,
  clockZeroElapsedSeconds,
  totalSecondsPlayed,
}) {
  if (!Array.isArray(clockSamples) || typeof initialClockSeconds !== "number") {
    return [];
  }
  ...
  for (const sample of clockSamples) {
    if (
      typeof sample.replayTime !== "number" ||
      typeof sample.gameSecondsRemaining !== "number"
    ) {
      continue;
    }
    ...
    const clockElapsed =
      overtimeElapsed ??
      initialClockSeconds - sample.gameSecondsRemaining;
...
function createEventBase({
  ...
  const clockDerivedElapsed =
    ...
    typeof gameSecondsRemaining === "number"
      ? initialClockSeconds - gameSecondsRemaining
      : null;

  // During overtime the game clock is stuck at 0:00. Correctly compute elapsed
  // game time as initialClockSeconds + seconds since the clock first hit zero.
...
function incrementPlayerSummary(playerSummary, playerName, team, type) {
  ...
  if (summary.team === null || summary.team === undefined) {
    summary.team = team;
  }
...
function getDemoPairKey(event) {
  if (event.type === "kill") {
    return `${event.playerName}|${event.victimPlayerName ?? "unknown"}`;
  }

  if (event.type === "death") {
    return `${event.killerPlayerName ?? "unknown"}|${event.playerName}`;
  }
...
function isCounterCoveredByRawDemo(event, rawDemoEvents, windowSeconds = 3) {
  ...
  return rawDemoEvents.some((rawEvent) => {
    ...
    if (rawEvent.type === "kill" && rawEvent.playerName !== event.playerName) {
      return false;
    }
    if (rawEvent.type === "death" && rawEvent.playerName !== event.playerName) {
      return false;
    }
...
function dedupeTimelineEvents(events) {
  ...
  for (const event of events) {
    if (
      event.source === "replicated-demolish" &&
  ...
  return deduped.filter(
    (event) => !isCounterCoveredByRawDemo(event, keptRawDemoEvents),
  );
}
...
async function main() {
  ...
  function addRawDemolishFallbackEvents({
    frameIndex,
    replayTime,
    victimCarActorId,
    updateObjectName,
    ...
    const numbers = findNumbers(attribute).filter(
      (number) => number !== victimCarActorId,
    );
    const attackerPriActorId = resolvePriFromReferencedNumbers(numbers);

    const victim = resolvePlayer(victimPriActorId);
    const attacker =
      attackerPriActorId !== null ? resolvePlayer(attackerPriActorId) : null;
    ...
    rawDemoEvents.push({
      ...
      attackerPriActorId,
      attackerPlayerName: attacker?.playerName ?? null,
    ...
    const deathEvent = {
      ...
      team: victim.team,
      killerPlayerName: attacker?.playerName ?? null,
      killerTeam: attacker?.team ?? null,
    };
  ...
  for (let frameIndex = 0; frameIndex < frames.length; frameIndex++) {
    ...
    let latestClockSample =
      clockSamples.length > 0 ? clockSamples[clockSamples.length - 1] : null;
    ...
    for (const update of frame.updated_actors ?? []) {
      ...
      if (DEMO_SIGNAL_NAMES.has(updateObjectName)) {
        incrementMapCount(demoSignalCounts, updateObjectName);
      }
      ...
      if (updateObjectName === "TAGame.GameEvent_Soccar_TA:SecondsRemaining") {
        ...
        if (typeof secondsRemaining === "number") {
          latestClockSample = {
            frameIndex,
            replayTime,
            gameSecondsRemaining: secondsRemaining,
          };
      ...
      if (updateObjectName === STATE_NAME_UPDATE) {
        ...
        if (stateName && stateSamples[stateSamples.length - 1]?.stateName !== stateName) {
          stateSamples.push({
            frameIndex,
            replayTime,
            elapsedSeconds: roundSeconds(replayTime - firstFrameTime),
            stateName,
          });
        }
      }
      ...
      if (activeObjectName === PRI_OBJECT_NAME) {
        ...
        if (eventConfig) {
          ...
          const initialClockSeconds = deriveInitialClockSeconds(
            clockSamples,
            300,
          );
      ...
      if (
        activeObjectName === CAR_OBJECT_NAME &&
        DEMO_UPDATE_NAMES.has(updateObjectName)
      ) {
        const initialClockSeconds = deriveInitialClockSeconds(
          clockSamples,
          300,
        ...
        addRawDemolishFallbackEvents({
          frameIndex,
          replayTime,
          victimCarActorId: actorId,
          updateObjectName,
          attribute,
          clockSample: latestClockSample,
          initialClockSeconds,
  ...
  for (const goal of goals) {
    ...
    const clockSample =
      typeof frameIndex === "number"
        ? findClockAtFrame(clockSamples, frameIndex)
        : null;

    const team = goal.PlayerTeam ?? null;

    if (team !== null) {
      scoreByTeam.set(team, (scoreByTeam.get(team) ?? 0) + 1);
    }
    ...
    const event = {
      ...createEventBase({
        ...
        initialClockSeconds,
        clockZeroElapsedSeconds,
        source: "replay-header",
      }),
      playerName: goal.PlayerName ?? "UNKNOWN",
      team,
      scoreAfter: {
        team0: scoreByTeam.get(0) ?? 0,
        team1: scoreByTeam.get(1) ?? 0,
      },
    };
  ...
  timelineEvents.sort((a, b) => {
    ...
    return (
      (eventTypeOrder.get(a.type) ?? 99) - (eventTypeOrder.get(b.type) ?? 99)
    );
  });

  const dedupedTimelineEvents = dedupeTimelineEvents(timelineEvents);
  const finalFrameElapsedSeconds =
    frameTimes.length > 0 ? Math.max(...frameTimes) - firstFrameTime : null;
  ...
  const ignoredCountdownIntervals = buildCountdownIntervals(
    stateSamples,
    finalFrameElapsedSeconds,
  );

  if (isOvertime) {
    applyOvertimeClockElapsed(
      dedupedTimelineEvents,
      overtimeStartSample,
      initialClockSeconds,
    );
  }
  ...
  const dedupedPlayerSummary = buildPlayerSummary(
    headerPlayers,
    dedupedTimelineEvents,
  );
  ...
  const output = {
    ...
    timeBasis: {
      ...
      overtimeStartReplayTimeSeconds: roundSeconds(
        overtimeStartSample?.replayTime,
      ),
  ...
  console.log("\nGame timeline:");
  console.table(
    dedupedTimelineEvents.map((event) => ({

...
async function main() {
  ...
  const players = [...priInfo.keys()]
    ...
    .map((priActorId) => {
      ...
      return {
        ...
        airActivations: airActivateByPri.get(priActorId) ?? null,
  ...
  const output = {
    ...
    notes: [
      ...
      "airActivations is the max AirActivateCount from the Jump component — counts every air-button press while airborne (first double-jump, flips, and air rolls combined). Not separable into individual types from this signal alone.",
  ...
  console.table(
    players.map((p) => ({
      ...
      "Air Activations": p.airActivations ?? "n/a",
